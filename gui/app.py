...
class AnalizadorAudioApp:
    ...
    def cargar_datos(self, r_real: Path, r_ia: Path):
        ...
        for i in range(self.num_ventanas):
            ...
            if self.eje_freqs is None:
                self.eje_freqs = freqs
            ...
            estado = (
                "Alta Coincidencia"
                if sim_pct > 70
                else ("Similitud Media" if sim_pct > 30 else "Desfase / IA Alterada")
            )

            self.resultados_ventanas.append({
                "ventana": i + 1,
                "inicio": i * self.tam_ventana,
                "fin": (i + 1) * self.tam_ventana,
                "similitud": sim_pct,
                "mse": mse,
                "estado": estado,
            })
    ...
    def construir_tab_individual(self):
        ...
        ttk.Combobox(ctrl_frame, textvariable=self.senal_var,
                     values=["Original", "IA Clon"], state="readonly").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Scale(ctrl_frame, from_=10, to=500, variable=self.muestreo_var,
                  orient=tk.HORIZONTAL, length=200).pack(side=tk.LEFT, padx=5)

        ttk.Button(ctrl_frame, text="Actualizar Análisis",
                   command=self.actualizar_grafico_individual).pack(side=tk.LEFT, padx=20)
    ...
    def actualizar_grafico_individual(self):
        ...
        datos = (self.a_real[centro:fin] if self.senal_var.get() == "Original"
                 else self.a_ia[centro:fin])
        ...
        self.ax_onda.stem(tiempo_muestreado, datos_muestreados,
                          linefmt=color, markerfmt="o", basefmt=" ",
                          label=f"Muestreo (1 cada {factor})")
    ...
    def construir_tab_tabla(self):
        columnas = ("Ventana", "Tiempo", "Similitud Temporal",
                    "Error MSE (Espectro)", "Veredicto del Sistema")
        self.tree = ttk.Treeview(self.tab_tabla, columns=columnas,
        ...
        for res in self.resultados_ventanas:
            ...
            self.tree.insert("", tk.END,
                             values=(res["ventana"], tiempo_str, sim_str,
                                     mse_str, res["estado"]))

        scrollbar = ttk.Scrollbar(self.tab_tabla, orient=tk.VERTICAL,
